...
function formatData(rawData, passages, filterString) {
  ...
  for (var index=0; index<data.results.length; index++) {
    ...
    if (dataItem.enriched_text && Array.isArray(dataItem.enriched_text)) {
      dataItem.enriched_text =dataItem.enriched_text[0];
    }
    ...
    var initialTextLen = (dataItem.text.length > 400)? 400 : dataItem.text.length;
    ...
    var newResult = {
      ...
      highlight: {
        ...
        subtitleIndexes: new Array()   // hold all start and end indexes of highlighted text in subtitle
      }
    };

    if (dataItem.extracted_metadata) {
      if (SHOW_FILE_NAME)
        newResult.filename = dataItem.extracted_metadata.filename;
      if (SHOW_LINK)
        newResult.link = LINK_PREFIX + dataItem.extracted_metadata.filename;
    }
    ...
    if (passages.results) {
      ...
      for (var i=0; i<passages.results.length; i++) {
        ...
        if (res.document_id === dataItem.id) {
          ...
          if (res.field === 'text') {
            ...
            newResult.textBlurb = res.passage_text;
          } else {
      ...
      if (dataItem.highlight) {
        ...
        if (dataItem.highlight.subtitle) {
          // title highlights
          processHighLights(dataItem.highlight.subtitle, dataItem.subtitle, newResult, 'subtitle');
        }
      }
...
function processHighLights(highlights, searchStr, newResult, type) {
  highlights.forEach(function(textStr) {
    ...
    if (type === 'text') {
      if (newResult.highlight.text) {
        newResult.highlight.text += '...<br/>...';
        newResult.highlight.text += textStr;
      } else {
        newResult.highlight.text = textStr;
      }
    } else if (type === 'subtitle') {
      newResult.highlight.subtitle = textStr;
      newResult.highlight.showHighlight = true;
    } else {
      newResult.highlight.title = textStr;
      newResult.highlight.showHighlight = true;
    }
    ...
    if (startIdx >= 0) {
      ...
      if (type === 'text') {
        ...
        newResult.highlight.subtitleIndexes.push({
          startIdx: startIdx,
          endIdx: startIdx + highlightLen
        });
      } else {
  ...
  if (type === 'text') {
    ...
    newResult.highlight.subtitleIndexes.sort(function (a, b) {
      return a.startIdx - b.startIdx;
    });
  } else {
